...
class DNN_model_with_embedding():
    ...
    def __init__(self, name, train_df, valid_df, log_dir, save_dir,
                 emb_list, no_emb_list, max_emb_size, ema_decay, bn_epsilon):
        self.name = name
    ...
    def make_no_emb_array(self):
        ...
        """
        self.no_emb_pipeline = Pipeline([
            ('col_selector', DataFrameSelectColumns(self.no_emb_list)),
            ('num_encoder', DataFrameConvertNumeric()),
            ('std_scaler', StandardScaler())
        ])
        self.no_emb_train_array = self.no_emb_pipeline.fit_transform(
            self.train_df)
        self.no_emb_valid_array = self.no_emb_pipeline.transform(
            self.valid_df)
    ...
    def embedding_config(self):
        ...
        self.emb_config = [(len(set(self.emb_train_array[:, el])),
                            min(self.max_emb_size,
                                (len(set(self.emb_train_array[:, el]))
                                 + 1) // 2))
    ...
    def get_embedding_tensor(self, tensor, emb_config, idx):
        ...
        """
        input_size, emb_dim = emb_config[idx]
    ...
    def validate_loss(self, i, valid_batch_size):
        ...
        for j in range(n_batches):
            ...
            X_emb = self.emb_valid_array[
                j*valid_batch_size: (j+1)*valid_batch_size]
            X_no_emb = self.no_emb_valid_array[
                j*valid_batch_size: (j+1)*valid_batch_size]
